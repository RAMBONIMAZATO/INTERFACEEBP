    ...
    <div class="dashboard-main-wrapper">
       ...
       <?php include('../config/template/header.php'); ?>
        ...
        <?php
                                            ...
                                            $p_abs_jours = "
                                            SELECT distinct DeptId as DeptId, Code,  Effectif, DATE_FORMAT(Dates,'%d/%m/%Y') AS Dates, Nb_pres, Nb_abs, ROUND(P_abs, 2) AS P_abs, C, CM, AA, P,Com, Susp, RM, Recup,HP,SM 
                                            FROM t_pourcentage_abs WHERE (Dates=date_add(curdate(), interval -1 day)) GROUP BY DeptId";
                                        ...
                                        ?>        
                                                    <tr>
                                                        ...
                                                        <td><?php echo $row["Nb_abs"]-$row["C"]-$row["CM"]-$row["AA"]-$row["P"]-$row["Com"]-$row["Susp"]-$row["RM"]-$row["Recup"]-$row["HP"]; ?></td>
                                                    </tr>
                                        ...
                                        <?php
                                            ...
                                            $bd = mysqli_select_db($connection, 'ebp');
                                            $q_sum = "
                                                    SELECT ROUND(sum(Nb_pres),0) AS T_pres, ROUND(sum(Nb_abs),0) AS T_abs, ROUND((sum(p_abs)), 2) AS T_p FROM t_abs WHERE (Dates=date_add(curdate(), interval -1 day)) AND Code!='STC')";
