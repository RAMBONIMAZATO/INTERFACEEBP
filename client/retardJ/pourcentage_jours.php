...
<head>
    ...
    <title>Pourcentage retard journalier</title>
    <!-- Bootstrap CSS -->
        ...
        <div class="card-body">
            <div class="table-responsive">
                <?php  
                    ...
                    $p_j_journalier = "SELECT DISTINCT DeptId AS DeptId, Code, Effectif, Nb_Ret, ROUND(P_Ret, 1) AS P_Ret, Dates 
                                        FROM t_pourcentage_retard_journalier WHERE (Dates=Date(now())) AND (Code!='STC')";
                    $r_p_j_journalier = mysqli_query($connection, $p_j_journalier);
                ...
                <table id="table_data" class="table table-striped table-bordered second" style="text-align: center;">
                    ...
                    <tbody>
                    <?php 
                        while ($row = mysqli_fetch_array($r_p_j_journalier)) {
                    ?>
                                <tr>
                    ...
                    <?php
                        ...
                        $bd = mysqli_select_db($connection, 'ebp');
                        $q_sum = "
                                SELECT sum(Effectif) AS T_eff,sum(Nb_Ret) AS T_ret, ROUND((sum(Nb_Ret)*100/sum(P_Ret)), 1) AS T_p FROM t_pourcentage_retard_journalier WHERE (Dates=Date(now()) AND Code!='STC')";
                    ...
                    ?>        
                            ...
                            <td><a href="retard.php"><?php echo $row['T_ret']; ?></a></td>
