    ...
    <div class="dashboard-main-wrapper">
       ...
       <?php include('../config/template/header.php'); ?>
        ...
        <div class="card-header">
            <h2 class="pageheader-title">SUMMARY MONTH</h2>
        ...
        <?php                         
                ...
                $query = "SELECT id, Code, TotalEmp, Nb_pres, Nb_temp, Nb_abs
                            FROM sumary WHERE (Dates=Date(now()) AND (Code IS NOT NULL)) ORDER BY id ASC";
                                ...
                                <div class="card-body">
                                    <table id="table_data" class="table table-striped table-bordered second" style="text-align: center;">
                                        ...
                                        <?php
                                            ...
                                            $bd = mysqli_select_db($connection, 'ebp');
                                            $q_sum = "
                                                    SELECT ROUND(sum(Effectif),0) AS T_eff, ROUND(sum(Nb_pres),0) AS T_pres, ROUND(sum(Nb_abs),0) AS T_abs, ROUND((sum(Nb_abs)*100)/(sum(Effectif)),2) AS T_p FROM t_abs WHERE (Dates=Date(now()))";
