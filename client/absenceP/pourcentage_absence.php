    ...
    <div class="dashboard-main-wrapper">
       ...
       <?php include('../config/template/header.php'); ?>
        ...
        <?php
          ...
          /*  $p_abs_jours = "
            SELECT (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(id) ELSE id END) AS id, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN 'BRODERIE' ELSE Code END) AS Code, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(EffJ_1) ELSE EffJ_1 END) AS EffJ_1,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(EffJ) ELSE EffJ END) AS EffJ,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Nb_pres) ELSE Nb_pres END) AS Nb_pres, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Nb_abs) ELSE Nb_abs END) AS Nb_abs , 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Conge) ELSE Conge END) AS Conge,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(CM) ELSE CM END) AS CM,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(AbsAuto) ELSE AbsAuto END) AS AbsAuto,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Perm) ELSE Perm END) AS Perm,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Com) ELSE Com END) AS Com ,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Susp) ELSE Susp END) AS Susp, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(RM) ELSE RM END) AS RM, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Recup) ELSE Recup END) AS Recup,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Hosp) ELSE Hosp END) AS Hosp,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(Map) ELSE Map END) AS Map,
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN SUM(NS) ELSE NS END) AS NS, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(SM) ELSE SM) AS SM, 
            (CASE WHEN (Code='BDR' OR Code='BDRPS') THEN sum(P_abs) ELSE P_abs END) AS P_abs 
            FROM t_pourcentage_cdd_cdi WHERE Dates = DATE( NOW( ) ) ORDER BY id";*/
...
FROM t_pourcentage_abs WHERE (Dates=Date(now()) AND Code!='EXP' AND Code!='GNBR') GROUP BY DeptId -->
            ...
            <table id="table_data" class="table table-striped table-bordered second" style="text-align: center;">
                <thead>
                  <tr>
                   <th>Id</th><th>Dept</th><th>EffJ-1</th><th>Eff</th><th>Pres</th><th>Abs</th><th>Conge</th><th>Mat</th><th>AbsAuto</th><th>Perm</th>
                   <th>Com</th><th>Susp</th><th>RM</th><th>Recup</th><th>Hosp</th><th>MaP</th><th>NS</th><th>SM</th><th>%Abs</th><th>%AbsSM</th>
                  </tr>
                </thead>
                <tbody>
            <?php 
