...
<!-- ######################################################################################################################################################### -->
                    ...
                    <?php  
                        ...
                        if (isset($_POST['btn_journalier'])) {
							...
							SELECT UserId, Name, Fonction, DeptId, DeptCode, DeptEff, Dates, Motif FROM t_present_jours WHERE (Dates=Date(now()) AND Motif IS NOT NULL)
							";
                            ...
                            $q_absence_journalier = "
                                ...
                                SELECT DISTINCT UserId, Name, Fonction, DeptId, Code, Effectif, DATE(NOW()) AS Dates
                            ...
                            if($r_q_journalier && $r_q_update_journalier && $r_q_pourcentage_retard_journalier && $r_q_absence_journalier && $r_q_pourcentage_absence_journalier && 
                                $r_insert_l_ret && $r_q_retard_permanant && $r_pourcentage_retard_permanant){
                                        echo '<div class="alert alert-success" role="alert">Les retards ont été insérées!</div>';
                                    }else{
                        ...
                        if (isset($_POST['btn_absence'])) {
                            $insert_l_abs = "INSERT INTO t_absence_jours(UserId, Name, Fonction, DeptId, Code,Effectif, Dates)
                            SELECT DISTINCT UserId, Name, Fonction, DeptId, Code, Effectif, DATE( NOW( ) ) AS Dates
                            ...
                            if($r_insert_l_abs){
                                echo '<div class="alert alert-success" role="alert">Les absents ont été enregistrées!</div>';
                            }else{
                                echo '<div class="alert alert-success" role="alert">Erreur d enregistrement des absents!</div>';
                            }
                        }
                        /**/
                            /*
                            SELECT DISTINCT u.DeptId as DeptId, u.Code as Code, u.Effectif, as Effectif, DATE(NOW()) as Dates, (u.Effectif-COUNT(DISTINCT(a.UserId))) as Nb_pres,
                            COUNT(DISTINCT a.UserId) as Nb_abs ,(COUNT(DISTINCT(a.UserId))*100/u.Effectif) as p_abs
                            FROM t_dept u LEFT JOIN t_absence_jours a ON u.DeptId=a.DeptId AND a.Dates=DATE(NOW())
                            GROUP BY u.DeptId";*/
                        if (isset($_POST['btn_pourcentage'])) {
                            /*Pourcentage absence*/
                            $insert_p_abs="INSERT INTO `t_abs`(DeptId, Code, Effectif, Dates, Nb_pres, Nb_abs, p_abs)
                            SELECT DISTINCT u.DeptId as DeptId, u.Code as Code, u.Effectif as Effectif,(CASE WHEN p.Dates IS NULL THEN DATE(NOW()) ELSE p.Dates END) AS Dates, 
                            ...
                            FROM  t_dept u LEFT JOIN t_present_jours p ON u.DeptId=p.DeptId AND p.Dates=DATE(NOW())
	                       ...
	                       /* $q2= "UPDATE t_h_nuit SET Pause='00:00:00' WHERE Dates=date_add(curdate(), interval -1 day)";
	                        $r_q2 = mysqli_query($connection, $q2);
	                        
	                        $q3="UPDATE t_h_nuit SET P_entree='22:00:00' WHERE Dates=date_add(curdate(), interval -1 day)";
	                        ...
	                        $q4="UPDATE t_h_nuit SET H_travail=(CASE WHEN H_entree <= P_entree THEN TIMEDIFF(ADDTIME(TIMEDIFF(H_sortie, P_entree), '24:00:00'), Pause) 
	                            WHEN H_entree > P_entree THEN TIMEDIFF(ADDTIME(TIMEDIFF(H_sortie, H_entree), '24:00:00'), Pause) END) WHERE Dates=date_add(curdate(), interval -1 day)";
	                        $r_q4 = mysqli_query($connection, $q4);*/
                            /*fin night shift  */
                            /* && $r_q_travail_employee && $r_q_update_entree_sortie && $r_entre_sortie && $r_q_update_pause && $r_q1 && $r_q2 && $r_q3 && $r_q4*/
                            if($r_insert_p_abs){
                                echo '<div class="alert alert-success" role="alert">Les pourcentages ont été inserées!</div>';
                            }else{
                                echo '<div class="alert alert-success" role="alert">Erreur d enregistrement des pourcentages!</div>';
                            }
                        }
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form id="validationform" data-parsley-validate="" novalidate="" action="insert_data.php" method="post">
                                        ...
                                        <button class="btn btn-primary" type="submit" name="btn_absence">ABSENCE</button>
                                        <button class="btn btn-primary" type="submit" name="btn_pourcentage">POURCENTAGE</button>
                                    </form>
                                </div>
                            </div>
                        </div>
...
############################################ HEURES NIGHT SHIFT###################################################################################
                    if (isset($_POST['btn_sup_motif'])) {
                        ...
                        /*$qt_h_travail="DELETE FROM t_h_travail WHERE Dates=date_add(curdate(), interval -1 day)";
                        $r_qt_h_travail=mysqli_query($connection, $qt_h_travail);

                        $qt_h_nuit="DELETE FROM t_h_nuit WHERE Dates=date_add(curdate(), interval -1 day)";
                        ...
                        if($r_qt_abs && $r_qmotif && $r_qconge && $r_qcongemat && $r_qrepmed && $r_qabsauto && $r_qpermission && $r_qsuspendu && $r_qrecup && $r_qcomission && $r_qhospitalise && 
                            $r_qmiseapied && $r_qnightshift && $r_qsansmotif && $r_qrepmed && $r_qpourabs && $r_qpourccdi &&  $r_qsum){
                            echo '<div class="alert alert-danger" role="alert">The data is deleted!</div>';
                        }else{
...
########################################################REQUETE POURCENTAGE D'ABSENCE ############################################
            ...
            SELECT 'Total CDI' AS Code, sum(EffJ_1), sum(EffJ), sum(Nb_pres), sum(Nb_abs), ROUND((sum(Nb_abs)*100/sum(EffJ)), 1), sum(Conge), sum(CM), sum(AbsAuto), sum(Perm), sum(Com), sum(Susp), sum(RM), sum(Recup), sum(Hosp), sum(Map), sum(NS), sum(SM), Dates 
            ...
            SELECT  'Total CDD'AS Code, sum(EffJ_1), sum(EffJ), sum(Nb_pres), sum(Nb_abs), ROUND((sum(Nb_abs)*100/sum(EffJ)), 1), sum(Conge), sum(CM), sum(AbsAuto), sum(Perm), sum(Com), sum(Susp), sum(RM), sum(Recup), sum(Hosp), sum(Map), sum(NS), sum(SM), Dates FROM `t_pourcentage_cdd_cdi` 
...
################################################################REQUETE MISE EN FORME RESULTATS ##########################################################
            ...
            SELECT 'ADM' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres, SUM(Nb_abs) AS Nb_abs,SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            ...
            SELECT 'BRODERIE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs,  SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'COMPLIANCE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'COUPE MK 01 ET MK3' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'DRY-PROCESS' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'FINITION' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'GENERAL' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'IE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'LAVERIE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'MAINTENANCE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'MARKER' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'MECANO' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'QA' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'RECYCLING' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'SAMPLE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'SEWING' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'STORE' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'TRAINING' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT 'ZIPPER' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, SUM(Conge) AS Conge, SUM(CM) AS CM,
            SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            ...
            SELECT (CASE WHEN Code IS NOT NULL THEN  'Total CDI' ELSE 'Total CDI' END) AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, 
            SUM(Conge) AS Conge, SUM(CM) AS CM, SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, 
            SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            WHERE (Code= 'Total CDI') AND Dates='$txtEndChoix'";*/
            $q20="INSERT INTO t_pourcentage_absence(Code, EffJ_1, EffJ, Nb_pres, Nb_abs, P_abs, Conge, CM, AbsAuto, Perm, Com, Susp, RM, Recup, Hosp, Map, NS, SM, Dates) 
            SELECT 'Total CDI' AS Code, SUM(EffJ_1) AS EffJ_1, SUM(EffJ) AS EffJ, SUM(Nb_pres) AS Nb_pres,SUM(Nb_abs) AS Nb_abs, SUM(P_abs) AS P_abs, 
            SUM(Conge) AS Conge, SUM(CM) AS CM, SUM(AbsAuto) AS AbsAuto, SUM(Perm) AS Perm, SUM(Com) AS Com, SUM(Susp) AS Susp, SUM(RM) AS RM, SUM(Recup) AS Recup, SUM(Hosp) AS Hosp, SUM(Map) AS Map, 
            SUM(NS) AS NS, SUM(SM) AS SM, Dates
            FROM  `t_pourcentage_cdd_cdi` 
            WHERE (Code= 'Total CDI') AND Dates='$txtEndChoix'";
