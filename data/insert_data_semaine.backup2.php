...
########################################### LES HEURES CONGES #########################################################################################################
            ...
            abs.H_autoabs AS H_autoabs, com.H_commission AS H_commission, m.H_miseapied AS H_miseapied 
            FROM t_dept_user t 
            ...
            abs.H_autoabs AS H_autoabs, com.H_commission AS H_commission, m.H_miseapied AS H_miseapied 
            FROM t_dept_user t 
            LEFT JOIN t_h_conge c ON t.UserId=c.UserId AND c.Name=t.Name
            LEFT JOIN t_h_repos r ON t.UserId=r.UserId AND r.Name=t.Name
            LEFT JOIN t_h_permission p ON t.UserId=p.UserId AND p.Name=t.Name
            LEFT JOIN t_h_autoabs abs ON t.UserId=abs.UserId AND abs.Name=t.Name
            LEFT JOIN t_h_commission com ON t.UserId=com.UserId AND com.Name=t.Name
            LEFT JOIN t_h_miseapied m ON t.UserId=m.UserId AND m.Name=t.Name
            ...
            AND (c.Dates='$StartDate') AND (r.Dates='$StartDate') AND (p.Dates='$StartDate') AND (abs.Dates='$StartDate') AND (com.Dates='$StartDate') AND (m.Dates='$StartDate')
            GROUP BY t.UserId";
...
############################################ HEURES CONGE JOURS NUIT DIMANCHE ############################################################################################
            ...
            FROM t_dept_user t
            LEFT JOIN t_h_travail j ON t.UserId = j.UserId AND t.Name=j.Name 
            LEFT JOIN t_h_nuit n ON t.UserId=n.UserId AND t.Name=n.Name
            WHERE j.Dates='$StartDate' AND n.Dates='$StartDate' 
            ...
            (CASE 
                ...
                WHEN c.Dates IS NOT NULL THEN c.Dates 
            END) AS Dates, j.H_jours AS H_jours, j.H_nuit AS H_nuit,
            ...
            FROM t_dept_user t
            LEFT JOIN t_jours_nuit j ON t.UserId=j.UserId AND t.Name=j.Name
            LEFT JOIN t_jours_conge c ON t.UserId=c.UserId AND t.Name=c.Name
            WHERE j.Dates='$StartDate' AND c.Dates='$StartDate'
            GROUP BY  t.UserId, t.Name";
...
############################################ HEURES CONGE JOURS NUIT DIMANCHE ############################################################################################
            ...
            (CASE 
                WHEN j.Dates IS NOT NULL THEN j.Dates 
                WHEN c.Dates IS NOT NULL THEN c.Dates 
            END) AS Dates, j.H_jours AS H_jours, j.H_nuit AS H_nuit, j.H_dimanche AS H_ferie,
            c.H_conge AS H_conge, c.H_repos AS H_repos, c.H_permission AS H_permission, c.H_autoabs AS H_autoabs, c.H_commission AS H_commission, c.H_miseapied AS H_miseapied
            FROM t_dept_user t
            LEFT JOIN t_jours_nuit j ON t.UserId=j.UserId AND j.Dates='$StartDateFerie ' 
            LEFT JOIN t_jours_conge c ON t.UserId=c.UserId AND c.Dates='$StartDateFerie '
            GROUP BY  t.UserId, t.Name";
            $r_q_j_conge=mysqli_query($connection, $q_j_n_conge);
...
############################################ HEURES CONGE JOURS NUIT DIMANCHE ############################################################################################
            ...
            (CASE 
                WHEN j.Dates IS NOT NULL THEN j.Dates 
                WHEN c.Dates IS NOT NULL THEN c.Dates 
            END) AS Dates, j.H_jours AS H_jours, j.H_nuit AS H_nuit, j.H_dimanche AS H_dimanche,
            c.H_conge AS H_conge, c.H_repos AS H_repos, c.H_permission AS H_permission, c.H_autoabs AS H_autoabs, c.H_commission AS H_commission, c.H_miseapied AS H_miseapied
            FROM t_dept_user t
            LEFT JOIN t_jours_nuit j ON t.UserId=j.UserId AND j.Dates='$StartDateDimanche ' 
            LEFT JOIN t_jours_conge c ON t.UserId=c.UserId AND c.Dates='$StartDateDimanche '
            GROUP BY  t.UserId, t.Name";
            $r_q_j_conge=mysqli_query($connection, $q_j_n_conge);
