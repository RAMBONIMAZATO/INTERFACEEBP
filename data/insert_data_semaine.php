...
########################################### LES HEURES CONGES #########################################################################################################
            ...
            $h_repos="INSERT INTO t_h_repos(UserId, Name, Fonction, DeptId, Code, Dates, H_repos)
                        SELECT UserId, Name, Fonction, DeptId, Code, Dates, '08:00:00' AS H_repos FROM t_motif WHERE Obs='repos medical' AND Dates='$StartDate'";
...
############################################ HEURES CONGE JOURS NUIT DIMANCHE ############################################################################################
            ...
            /*les heures de travail a lobes*/
            $q_j_l="INSERT INTO t_jours_nuit(UserId, Name,Fonction, DeptId, Code, Dates, H_jours, H_nuit)
            SELECT j.UserId AS UserId, j.Name AS Name, j.Fonction AS Fonction, j.DeptId AS DeptId, j.Code AS Code, '$StartDate' AS Dates, j.H_travail AS H_jours, n.H_travail AS H_nuit
            FROM t_h_travail j 
            LEFT JOIN t_h_nuit n ON j.UserId=n.UserId AND j.Name=n.Name
            WHERE j.Dates='$StartDate' AND (j.Commentaire IS NULL) AND n.Dates='$StartDate'
            GROUP BY j.UserId, j.Name";
            $r_q_j_l=mysqli_query($connection, $q_j_l);

            /*les travailleur jours*/
            ...
            FROM t_jours_nuit
            WHERE Dates='$StartDate') AND j.Dates='$StartDate'";
            $r_q_jours=mysqli_query($connection, $q_jours);
            ...
            SELECT j.UserId AS UserId, j.Name AS Name, j.Fonction AS Fonction, j.DeptId AS DeptId, j.Code AS Code, '$StartDate' AS Dates, j.H_jours AS H_jours, j.H_nuit AS H_nuit,
            ...
            FROM t_jours_nuit j
            LEFT JOIN t_jours_conge c ON j.UserId=c.UserId AND j.Dates=c.Dates
            WHERE j.Dates='$StartDate'
            GROUP BY  j.UserId, j.Name";
            $r_q_j_n_conge=mysqli_query($connection, $q_j_n_conge);

            /*les heures de travail jours nuit*/
            $q_j_n="INSERT INTO t_jours_nuit_ferie_dimanche(UserId, Name,Fonction, DeptId, Code, Dates, H_jours, H_nuit)
            SELECT j.UserId AS UserId, j.Name AS Name, j.Fonction AS Fonction, j.DeptId AS DeptId, j.Code AS Code, j.Dates AS Dates, j.H_jours AS H_jours, j.H_nuit AS H_nuit
            FROM t_jours_nuit j
            WHERE NOT EXISTS(SELECT UserId, Name, Fonction, DeptId, Code, Dates, H_jours, H_nuit FROM t_jours_nuit_ferie_dimanche WHERE Dates='$StartDate') AND j.Dates='$StartDate'";
            $r_q_j_n=mysqli_query($connection, $q_j_n);

            /*les heures de travail jours nuit*/
            $q_j_conge="INSERT INTO t_jours_nuit_ferie_dimanche(UserId, Name,Fonction, DeptId, Code, Dates, H_conge, H_repos, H_permission, H_autoabs, H_commission, H_miseapied)
            SELECT c.UserId AS UserId, c.Name AS Name, c.Fonction AS Fonction, c.DeptId AS DeptId, c.Code AS Code, c.Dates AS Dates, c.H_conge AS H_conge, c.H_repos AS H_repos,
            c.H_permission AS H_permission, c.H_commission AS H_commission, c.H_miseapied AS H_miseapied 
            FROM t_jours_conge c
            WHERE NOT EXISTS(SELECT UserId, Name, Fonction, DeptId, Code, Dates, H_conge, H_repos, H_permission, H_autoabs, H_commission, H_miseapied 
            FROM t_jours_nuit_ferie_dimanche WHERE Dates='$StartDate') AND c.Dates='$StartDate'";
            $r_q_j_conge=mysqli_query($connection, $q_j_conge);

            if($r_h_conge && $r_h_repos && $r_h_permission && $r_h_autoabs && $r_h_commission && $r_h_miseapied && $r_q_jours_conge && $r_q_j_l && $r_q_jours && $r_q_j_nuit && $r_q_j_n_conge && $r_q_j_n && 
                $r_q_j_conge){
