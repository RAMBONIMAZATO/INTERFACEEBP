<?php  
	$connect =mysqli_connect("localhost", "root", "", "mklen");
	if (isset($_POST['item_name'])) {
		...
		for ($count=0; $count < count($item_name); $count++) { 
			...
			if ($item_name_clean != '' && $item_code_clean != '' && $item_desc_clean !='' && $item_price_clean !='') {
				$query .='
				INSERT INTO item(item_name, item_code, item_description, item_price) VALUES("'.$item_name_clean.'","'.$item_code_clean.'","'.$item_desc_clean.'", "'.$item_price_clean.'")';
			}
		}
		if ($query != '') {
