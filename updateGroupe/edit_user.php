<?php
/*$query="UPDATE Userinfo SET  Plage_E='$plageE', Plage_S='$plageS',Groupe='$groupe'
            WHERE Userid='$matUser'";
...
$connection = odbc_connect("mklen", "", "");
    if(isset($_POST["save"]) && $_POST["save"]!="") {
        ...
        for($i=0;$i<$usersCount;$i++) {
            ...
            $plageE=$_POST["txtPlageE"];
            $plageS=$_POST["txtPlageS"];
            $groupe=$_POST['txtGroupe']; 
            if(empty($groupe) && empty($plageS)){
                $query="UPDATE Userinfo SET Plage_E='$plageE', Plage_S=NULL, Groupe=NULL
                WHERE Userid='$matUser'";
                odbc_exec($connection, $query); 
            }elseif(empty($groupe)){
                $query="UPDATE Userinfo SET Plage_E='$plageE', Plage_S='$plageS', Groupe=NULL
                WHERE Userid='$matUser'";
                odbc_exec($connection, $query); 
            }elseif (empty($plageS)) {
                $query="UPDATE Userinfo SET Plage_E='$plageE', Plage_S=NULL, Groupe='$groupe'
                WHERE Userid='$matUser'";
                odbc_exec($connection, $query); 
            }else{
                $query="UPDATE Userinfo SET  Plage_E='$plageE', Plage_S='$plageS',Groupe='$groupe'
        ...
        <div class="card-body">
            <form name="frmUser" method="post" action="">
                    ...
                    <table id="table_data" class="table table-striped table-bordered second" style="text-align: center;">
                        ...
                        <?php
                            ...
                            for($i=0;$i<$rowCount;$i++) {
                                $q="SELECT Userinfo.Userid AS Userid, Userinfo.Name AS Name, Format(Userinfo.Plage_E, 'hh:mm:ss') AS Plage_E, Format(Userinfo.Plage_S, 'hh:mm:ss') AS Plage_S, Userinfo.Groupe AS Groupe
                                FROM Userinfo WHERE Userid='" . $_POST["users"][$i] . "'";
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="Plage_S[]" class="txtField" value="<?php echo $row[$i]['Plage_S']; ?>"></td>
                            <td><input type="text" name="Groupe[]" class="txtField" value="<?php echo $row[$i]['Groupe']; ?>"></td>
                        </tr>
                        <?php
                            }
                        ?>
                ...
                <div class="card-footer">
                        <label>PlageE : </label>
                        <input type="text" name="txtPlageE">
                        <label>PlageS : </label>
                        <input type="text" name="txtPlageS">
                        <label>Groupe : </label>
                        <input type="text" name="txtGroupe">
