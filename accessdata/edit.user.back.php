...
if(isset($_POST["save"]) && $_POST["save"]!="") {
	...
	for($i=0;$i<$usersCount;$i++) {
        $plage='08:00:00';
        $query="UPDATE Userinfo SET  Plage_E=$plage WHERE Userid='" . $_POST["Userid"][$i] . "'";
        odbc_exec($connection, $query) or die ("Error Execute [".$query."]");  
	}
	header("Location: update_plage.php");
}
			...
			<form name="frmUser" method="post" action="">
				<div class="table-responsive">
					<table id="table_data" class="table table-striped table-bordered second" style="text-align: center;">
						...
						?>
						<tr>
							<td><label><?php echo $row[$i]['Userid']; ?></label><input type="hidden" name="Userid[]" class="txtField" value="<?php echo $row[$i]['Userid']; ?>"></td>
							<td><label><?php echo $row[$i]['Name']; ?></label></td>
