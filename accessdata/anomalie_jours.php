...
/*$query ="SELECT Checkinout.Logid AS Logid, Userinfo.Userid AS UserId, Userinfo.Name AS Name, Userinfo.Duty AS Duty, Userinfo.Deptid AS Deptid, t_dept_save.Code AS Code, 
t_dept_save.Effectif AS Effectif, Format(DateValue(Checkinout.CheckTime), 'yyyy-mm-dd') AS Dates, Format(TimeValue(Checkinout.CheckTime), 'hh:mm:ss') AS H_E
FROM Userinfo, t_dept_save, Checkinout
WHERE (((Userinfo.Deptid)=[t_dept_save].[DeptId]) AND ((t_dept_save.Code)<>'STC') AND ((Checkinout.Userid)=[Userinfo].[Userid]) AND (Checkinout.Userid='20036') AND ((DateValue([Checkinout].[CheckTime]))=(Date()))) ";	*/
$con = odbc_connect("mklen", "", "");


$query ="SELECT Checkinout.Logid AS Logid, Checkinout.Userid AS UserId, Checkinout.CheckTime AS CheckTime, Format(DateValue(Checkinout.CheckTime), 'yyyy-mm-dd') AS Dates, 
Format(TimeValue(Checkinout.CheckTime), 'hh:mm:ss') AS H_E
FROM Checkinout
WHERE (((DateValue([Checkinout].[CheckTime]))=(Date()-1))) 
